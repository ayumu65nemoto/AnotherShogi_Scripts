...
public class Effect6 : MonoBehaviour
{
    ...
    private string[] _announce = {"ひみつゾーン（仮）に送る持ちゴマを　選択してください", "送り先の　ひみつゾーン（仮）を　選択してください"};
    ...
    void Start()
    {
        ...
        fieldManager.OpenAnnounce(_announce[0]);
    }
                    ...
                    if (hit.collider.gameObject.tag == "1P" || hit.collider.transform.parent.gameObject.tag == "1P")
                    {
                        if (hit.collider.transform.parent.gameObject.tag == "1P")
                        {
                            ...
                            if (_piece.fieldStatus == PieceController.FieldStatus.OnBoard)
                            {
                                ...
                                _isSelectPiece = true;
                                fieldManager.OpenAnnounce(_announce[1]);
                            }
                            else
                            {
